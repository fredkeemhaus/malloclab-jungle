...
static void *find_fit(size_t asize) { // first fit 검색을 수행
    void *bp;
    for (bp= heap_listp; GET_SIZE(HDRP(bp)) > 0; bp = NEXT_BLKP(bp)) { // init에서 쓴 heap_listp를 쓴다. 처음 출발하고 그 다음이 regular block 첫번째 헤더 뒤 위치.
        // for문이 계속 돌면 epilogue header까지 간다. epilogue header는 0이니까 종료가 된다.
...
static void place(void *bp, size_t asize) { // 들어갈 위치를 포인터로 받는다.(find fit에서 찾는 bp) 크기는 asize로 받음.
    ...
    size_t currSize = GET_SIZE(HDRP(bp)); // 현재 있는 블록의 사이즈.
    if ( (currSize - asize) >= (2 * DSIZE)) { // 현재 블록 사이즈안에서 asize를 넣어도 2 * DSIZE(헤더와 푸터를 감안한 최소 사이즈)만큼 남냐? 남으면 다른 data를 넣을 수 있으니까.
        ...
        PUT(HDRP(bp), PACK(currSize - asize, 0)); // 나머지 블록은(currSize - asize) 다 가용하다(0)하다라는걸 다음 헤더에 표시.
        PUT(FTRP(bp), PACK(currSize - asize, 0)); // 푸터에도 표시.
    }
    else{
        PUT(HDRP(bp), PACK(currSize, 1)); // 위의 조건이 아니면 asize만 currSize에 들어갈 수 있으니까 내가 다 먹는다.
...
static void *extend_heap(size_t words) { // 새 가용 블록으로 힙 확장, 예시로는 2의 10승만큼 워드블록을 만들어라. ( 1<<12 / 4(word)) = 1024
    char *bp;
...
int mm_init(void) { // 처음에 heap을 시작할 때 0부터 시작. 완전 처음.(start of heap)
    ...
    // CHUNKSIZE = 4096 byte / WSIZE = 4 byte = 1024 byte
...
void *mm_malloc(size_t size) { // 가용 리스트에서 블록 할당 하기
    ...
    if (size <= DSIZE) {
        asize = MINBLOCK; // 헤더와 푸터를 포함해서 블록 사이즈를 다시 조정해야되니까 DSIZE의 2배를 준다.(헤더와 푸터 합쳐서 8바이트)예를 들어 헤더와 푸터에는 16/1 이 들어간다.
    } else {
        // asize = DSIZE * ( (size + (DSIZE) + (DSIZE - 1)) / DSIZE ); // size보다 클 때, 블록이 가질 수 있는 크기 중에 최적화된 크기로 재조정.
        asize = MAX(DSIZE + ALIGN(size), MINBLOCK); // ALIGN 매크로 사용
    }
...
void *mm_realloc(void *bp, size_t size) { // 크기를 조정할 블록의 위치와 요청 사이즈를 인자로 받는다. 
    if(size <= 0) { 
        mm_free(bp);
        return 0;
    }
    if(bp == NULL) {
        return mm_malloc(size); // malloc(size) 을 통해 요청 사이즈만큼 블록을 할당한다. 
    }
    ...
    memcpy(newptr, bp, oldsize); // 블록 내 데이터 또한 똑같이 옮겨야 하니 C라이브러리에 있는 memcpy 함수를 활용한다.
    mm_free(bp);
